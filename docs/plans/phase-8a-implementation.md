...
and the driver tested over a pty.
...
### Task 12: The ELM327 acceptance tests, exercised against a simulated dongle

**This is the one task that can be cut.** Cutting it leaves the nineteen-point acceptance
list as test code that has never executed, and a suite that has never run once is not
evidence — it may itself be broken, and bench day is an expensive place to find out. It is
last so that the decision can be taken with everything else already done.
...
the real simulator, with only the physical link simulated.
...
The dry run is the point of the task. Writing nineteen tests that have never
executed and calling that a deliverable would be the weakest part of Phase 8a: a
suite that has never run once is not evidence, and bench time is the most
expensive place to discover it was broken. The bridged fake speaks the AT
commands locally and forwards hex requests to a real ISO-TP socket, so the same
test bodies run against the real simulator with only the physical link simulated.
MSG
```
...
disprove, which is why it is thin and why Task 12 exists. The `hw_bench` pre-flight
(Task 6) has never run against a real interface; its `interface_index` and
`is_interface_up` calls are the same ones the integration conftest already uses on vcan,
but its refusal path on a physical interface is untested until 8b.
